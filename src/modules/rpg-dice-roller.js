...
(function (root, factory) {
  ...
  DiceRoller.notationPatterns = (function () {
     ...
     */
    strings.explode =
      "((!{1,2}p?)((" + strings.comparisonOperators + ")?([0-9]+))?)";
     ...
     */
    strings.addition =
      "(" + strings.arithmeticOperator + ")([1-9]+0?(?![0-9]*d)|H|L)";
    ...
    return {
      ...
      get: function (name, flags, matchWhole) {
        ...
        if (!regExp[cacheName]) {
          regExp[cacheName] = new RegExp(
            (matchWhole ? "^" : "") + strings[name] + (matchWhole ? "$" : ""),
            flags || undefined
          );
        }
  ...
  DiceRoller.parseNotation = function (notation) {
    ...
    if (notation) {
      ...
      var match;
      while (
        (match = DiceRoller.notationPatterns
          .get("notation", "g")
          .exec(notation)) !== null
      ) {
        var die = {
          ...
          sides: DiceRoller.utils.isNumeric(match[3])
            ? parseInt(match[3], 10)
            : match[3], // how many sides the die has - only parse numerical values to Int
        ...
        if (typeof die.sides === "string") {
          die.fudge =
            die.sides.match(
              DiceRoller.notationPatterns.get("fudge", null, true)
            ) || false;
        }
        ...
        if (match[10]) {
          ...
          var additionMatch;
          while (
            (additionMatch = DiceRoller.notationPatterns
              .get("addition", "g")
              .exec(match[10]))
          ) {
  ...
  var DiceRoll = function (notation) {
    ...
    var isComparePoint = function (comparePoint, value) {
      return comparePoint
        ? DiceRoller.utils.compareNumbers(
          value,
          comparePoint.value,
    ...
    var rollDie = function (die) {
      ...
      if (die.fudge) {
        ...
        callback = function (sides) {
          return diceRollMethods.fudge(
            DiceRoller.utils.isNumeric(die.fudge[1])
              ? parseInt(die.fudge[1])
              : 2
          );
        };
        ...
        parsedDice.forEach(function (item, index, array) {
          ...
          if (item.additions.length) {
            ...
            item.additions.forEach(function (aItem, aIndex, aArray) {
              ...
              dieTotal = DiceRoller.utils.equateNumbers(
                dieTotal,
                value,
                aItem.operator
              );
          ...
          // total the value
          total = DiceRoller.utils.equateNumbers(
            total,
            dieTotal,
            item.operator
          );
